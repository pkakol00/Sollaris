...
TEST(IOTest, SavesAndLoads) {
  ...
  std::vector<PlanetPosition> data2;
  PlanetData input;
  PlanetPosition posi;
  input.mass = 1.0;
  input.radious = -2.0;
  ...
  Vec3 v(1,1,1);
  posi.positions.push_back(v);
  posi.positions.push_back(v);
  posi.positions[1].x = 5;
  ...
  data.push_back(input);
  posi.planet_id = 0;
  data2.push_back(posi);
  posi.planet_id = 1;
  data2.push_back(posi);
  
  io.write_event(data,data2,"save.space");
  std::vector<PlanetData> new_data;
  std::vector<PlanetPosition> new_data2;
  io.read_event(new_data,new_data2,"save.space");
  ...
  EXPECT_EQ(data2[0].positions[1].x, new_data2[0].positions[1].x);
}
...
TEST(IOTest, WebDownloadAndParse) {
  IO io;
  std::vector<PlanetData> data1;
  std::vector<PlanetPosition> data2;
  io.get_web(data1,data2);
}